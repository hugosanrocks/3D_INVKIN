...
!
    ind(1,1)=indx
    ind(1,2)=indz+isignz*3
    dist(1)=ABS((h*ind(1,ico1)-(h*ind(1,ico2)* &
            pent+orde))*ABS(COS(radi)))
!
    ind(2,1)=indx+isignx
    ind(2,2)=indz+isignz*3
    dist(2)=ABS((h*ind(2,ico1)-(h*ind(2,ico2)* &
            pent+orde))*ABS(COS(radi)))
!
    ind(3,1)=indx+isignx*2
    ind(3,2)=indz+isignz*3
    dist(3)=ABS((h*ind(3,ico1)-(h*ind(3,ico2)* &
            pent+orde))*ABS(COS(radi)))
!
    ind(4,1)=indx+isignx*3
    ind(4,2)=indz+isignz*3
    dist(4)=ABS((h*ind(4,ico1)-(h*ind(4,ico2)* &
            pent+orde))*ABS(COS(radi)))
!
    ind(5,1)=indx+isignx*3
    ind(5,2)=indz+isignz*2
    dist(5)=ABS((h*ind(5,ico1)-(h*ind(5,ico2)* &
            pent+orde))*ABS(COS(radi)))
!
    ind(6,1)=indx+isignx*3
    ind(6,2)=indz+isignz
    dist(6)=ABS((h*ind(6,ico1)-(h*ind(6,ico2)* &
            pent+orde))*ABS(COS(radi)))
!
    ind(7,1)=indx+isignx*3
    ind(7,2)=indz
    dist(7)=ABS((h*ind(7,ico1)-(h*ind(7,ico2)* &
            pent+orde))*ABS(COS(radi)))
!
    MIN=10E4
    DO i=1,7
...
!
    IF (iseg == fseg) THEN
!     IF (delf < h) GO TO 200
!     IF (delf < 3.0*h) GO TO 200
      IF (delf < 4.0*h) GO TO 200
    ELSE
      IF (ico1 == 1) THEN
!       IF (delf <= h.AND.nfin > fin) GO TO 200
!       IF (delf <= 3.0*h.AND.nfin > fin) GO TO 200
        IF (delf <= 4.0*h.AND.nfin > fin) GO TO 200
      ELSE
!       IF (delf <= h.AND.nfin <= fin) GO TO 200
!       IF (delf <= 3.0*h.AND.nfin <= fin) GO TO 200
        IF (delf <= 4.0*h.AND.nfin <= fin) GO TO 200
