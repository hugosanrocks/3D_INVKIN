...
$region="-R-98/-92/13/18";
$region2="-R-117/-86/12.8/32";
...
$gpssta="~/svn/3D_INVKIN/3D_INVKIN/run/chi/plot/stations.sta";
$gpssta2="~/MEGA/INV3DKIN/run/jap/prepare/stations.sta2";
$gpssta2="~/MEGA/INV3DKIN/run/jap/prepare/stations.stause";
$coorsta="~/svn/3D_INVKIN/3D_INVKIN/run/chi/plot/stations.xy";
...
#Edge of the fault
#1
`psxy $projection $region -M -O -K -V -W15p,red edge_faultchi.out >> $psfile`;
#2
#`psxy $projection $region -M -O -K -V -W5p,blue edge_fault2.out >> $psfile`;

#all stations
`psxy $coorsta $region $projection -Si3.5 -Gwhite -W20,black -O -K -V >> $psfile`;
...
`psxy $epi $region $projection -Sa3.5 -Gred -W3,yellow -O -K -V >> $psfile`;

#Plotting basemap and coast and country lines
`pscoast $region2 $projection2 -X48 $ticks2 -P -N1 -Ggreen -Wblue -K -O -V -Dh -Q >> $psfile`;

`pscoast $region2 $projection2 -P -W1 -Gblack -Sgray -N1 -O -K -V >> $psfile`;
`psxy $projection2 $region2 -M -O -K -V -W30p,yellow squarechi.xy >> $psfile`;
