...
!PROGRAM USED ONLY TO CHECK THAT THE INVERSE 
!SUBROUTINE IS WORKING WELL (UNCOMMENT THE NEXT
!LINES)
...
!  A(0,0)= 2.25;
!  A(1,0)= 0; A(1,1)=2.25
!  A(2,0)= 0; A(2,1)=0; A(2,2)= 2.6666
!  A(3,0)= 0; A(3,1)=0; A(3,2)=0; A(3,3)= 2.666
...
subroutine inverse(A,n)
  ...
  !INPUT 
  !A =   Squared matrix positive definite
  !n =   dimension of matrix
  integer, intent(inout) :: n
  real*8, intent(inout) :: A(0:n-1,0:n-1)
  real*8, pointer :: A1(:,:), B(:,:), C(:,:)
  integer i, j, istat, Check_Matrix
  real*8 dhu, choldet
  character*1  answer

!  allocate(A(0:n-1,0:n-1),B(0:n-1,0:n-1),stat=istat)
  allocate(B(0:n-1,0:n-1),stat=istat)
