...
void store_rom(double F_H[2], double t, int check)
{
	...
	ofstream cp_rom;
	cp_rom.open(txt);
	cp_rom << setiosflags(ios::fixed) << setprecision(16) << F_H[0] << " " <<  setiosflags(ios::fixed) << setprecision(16) << F_H[1] << " " << setiosflags(ios::fixed) << setprecision(16) << t;
	cp_rom.close();
	delete [] txt;
}
...
int main(int argc, char *argv[])
{
	...
	cout << "\n \n Using Binomial Multi-Stage Offline Checkpointing for the approximate solution: \n \n" ;
		
	int info,j=0,snaps,snaps_in_ram,steps;
	enum ACTION::action whatodo;
	bool forward=true;
	

	cout << endl << "STEPS        -> number of time steps to perform" << endl;
	cout << "SNAPS        -> number of checkpoints (in RAM+in ROM)" << endl;
	cout << "SNAPS_IN_RAM -> number of checkpoints in RAM" << endl;
	...
	cout << " ENTER:   STEPS, SNAPS,SNAPS_IN_RAM, INFO \n";
	cin >> steps >> snaps >> snaps_in_ram >> info;
	...
	// Wir haben 3 Komponenten, da zusätzlich zu den beiden Zuständen
	...
	do
	{
		...
		if (whatodo == ACTION::takeshot) 
		{
			if(r->getwhere())
			{
				store(F,F_Check,t,r->getcheckram());
				
			}
			else
			{
				store_rom(F,t,r->getcheckrom());
			}
			if(info>1)
			{
		...
		if (whatodo == ACTION::restore)
		{
			...
			if(r->getwhere()) 
			{
				restore(F,F_Check,&t,r->getcheckram());
			}
			else
			{
				restore_rom(F,&t,r->getcheckrom());
			}
			if(info > 2)
			{
