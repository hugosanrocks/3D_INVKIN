...
for k=1:nsta
  for i=1:ncomp
    %read synthetics
    file=sprintf('syn_p/syn_p/syn_S%03d_C%01d.ascii',sort(k,2),i);
    syn2=load(file);
    syn(:,i)=syn2(:,2);
    file2=sprintf('syn_p/syn_np/syn_S%03d_C%01d.ascii',sort(k,2),i);
    syn2=load(file2);
    synnp(:,i)=syn2(:,2);
    %read observations
...
if (k <= 50)
  syn = syn + 1*(1.8);
  obs = obs + 1*(1.8);
  synnp=synnp+1*(1.8);%2 instead
else
  syn = syn + k*(1.6);
  obs = obs + k*(1.6);
  synnp=synnp+k*(1.6);%2 instead
end
  %write headers
