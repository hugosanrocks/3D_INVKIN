...
subroutine print_info(n,routine_type,optim,fcost,FLAG)
  ...
  if(optim%print_flag.eq.1) then
     ...
     if(FLAG.eq.'INIT') then
        ...
        write(optim%iunit,'(A30,ES10.2)') '     Convergence criterion  : ',optim%conv
        write(optim%iunit,'(A30,I7)')     '     Niter_max              : ',optim%niter_max
        write(optim%iunit,'(A30,ES10.2)') '     Initial cost is        : ',optim%f0
        write(optim%iunit,'(A30,ES10.2)') '     Initial norm_grad is   : ',ng
        call print_line()
        write(optim%iunit,'(A10,A10,A10,A10,A12,A12,A12)')&
             '   Niter   ' ,&
             '   fk      ' ,&
             '   ||gk||  ' ,&
             '     fk/f0   ' ,&
     ...
     elseif(FLAG.eq.'CONV') then
        ...
        if(optim%cpt_iter.ge.optim%niter_max) then
           write(optim%iunit,'(A50)') '  STOP: MAXIMUM NUMBER OF ITERATION REACHED    '
        else
           write(optim%iunit,'(A50)') '  STOP: CONVERGENCE CRITERION SATISFIED        '
        endif
        call print_line()
     elseif(FLAG.eq.'FAIL') then
        call print_line()
        write(optim%iunit,'(A50)') '  STOP: LINESEARCH FAILURE    '
        call print_line()
     else
        call print_infos()
     endif
...
contains
  ...
  subroutine open_file()
    ...
    if(FLAG.eq.'INIT') then
       open(optim%iunit, file=fname, status='REPLACE', action='WRITE', access='SEQUENTIAL', position='APPEND')
    else
       open(optim%iunit, file=fname, status='OLD',     action='WRITE', access='SEQUENTIAL', position='APPEND')
    end if
  end subroutine open_file
  
  subroutine print_line()
    write(optim%iunit,'(A70)') '**********************************************************************'
  end subroutine print_line
  
  subroutine print_huge(label)
    character (len=*), intent(in) :: label
    call print_line()
    write(optim%iunit,'(A50)') label
    call print_line()
  end subroutine print_huge
  
  subroutine print_infos()
    write(optim%iunit,'(I6,ES12.2,ES12.2,ES12.2,ES12.2,I8,I8)') &
         optim%cpt_iter,&          
         fcost,&
         ng,&
         fcost/optim%f0,&
