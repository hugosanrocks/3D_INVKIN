...
%debido a problemas numericos, esta parte es sencible
%se trata de encontrar el estrato para la onda incidente
%normalmente cualquier estrato deberia de ser equivalente 
%pero como hay productos entre exponenciales grandes y terminos muy pequeno
%se pierde precision
%tambien el det(A_DWN) no es exactamente 0, es una aproximacion
%por lo que los vectores propios son ligeramente diferentes
ics         = 1;
MAT         = para.reg(1).sub;
hc          = MAT(1).h;
lambda      = MAT(1).bet/real(para.fj);
% zrm         = min(zr0);
...
ics=max(ics+1,ncapas);

% while lambda>hc && ics<ncapas
%     ics     = ics+1;
%     hc      = hc+MAT(ics).h;
%     lambda  = MAT(ics).bet/real(para.fj);
% end
% ics=3;
