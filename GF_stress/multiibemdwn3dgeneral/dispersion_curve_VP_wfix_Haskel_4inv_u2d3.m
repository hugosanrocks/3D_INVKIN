function [vg,vp,f1,ikmax,kx]=dispersion_curve_VP_wfix_Haskel_4inv_u2d3(para,wi)
...
%                 for i=1:length(indf)
    ...
    while sum(abs(vp1-vp0)>err)>0
        ...
        if  pol==1
            ...
            det = mode_Rayleigh_2R(para,DWN1);
        end
        det(det==inf)=0;
    ...
    %falto modos!
    if pbw(j) == 1
        k20         = [k20;zeros(nmodej-length(indd1),1)]; %#ok<*AGROW>
    end
...
% figure(206);hold on;
% for j=1:nmode
%     plot(f1(j,1:ikmax(j)),vp(j,1:ikmax(j)),'m--')%
% end
% xlabel('f');ylabel('vp');
% 
% figure(207);hold on;
