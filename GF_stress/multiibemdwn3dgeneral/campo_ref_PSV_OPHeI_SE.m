function [u,t,sout]=campo_ref_PSV_OPHeI_SE(xs,zs,coord,kpi,ksi,kri,C,tipo_onda,kxk)
% funcion que da los campos de desplazamientos y las tractiones
% para Ondas Planas P-SV, homogenea e inhomogenea incidente en un semi espacio
...
% las ondas reflejadas seran incluidas
...
if tipo_onda==1
    ...
    kx      = kxk*kpi;
    [RPP,RPS,upi,upr,usr,kzP,kzS]=RPinc(C,kx,kpi,ksi);
    
    for i=1:2
        u_incP(i,:)  = upi(i)*    exp(-1i*(kx.*(x-xs)-kzP.*(z-zs)));
        u_refP(i,:)  = upr(i)*RPP*exp(-1i*(kx.*(x-xs)+kzP.*(z+zs)));
        u_refS(i,:)  = usr(i)*RPS*exp(-1i*(kx.*(x-xs)+kzS.*(z+zs)));
    end
...
elseif tipo_onda==2
    ...
    kx      = kxk*ksi;
    [RSP,RSS,usi,upr,usr,kzP,kzS]=RSinc(C,kx,kpi,ksi);
    for i=1:2
        u_incS(i,:)  = usi(i)*    exp(-1i*(kx.*(x-xs)-kzS.*(z-zs)));
        u_refP(i,:)  = upr(i)*RSP*exp(-1i*(kx.*(x-xs)+kzP.*(z+zs)));
        u_refS(i,:)  = usr(i)*RSS*exp(-1i*(kx.*(x-xs)+kzS.*(z+zs)));
    end
    u=u_incP+u_incS+u_refP+u_refS;
else
...
if tipo_onda==1 || tipo_onda==2
    ...
    du2dx1= (-1i*kx )*u(2,:);
    du1dx2= ( 1i*kzP)*u_incP(1,:) + ( 1i*kzS)*u_incS(1,:) + (-1i*kzP)*u_refP(1,:) + (-1i*kzS)*u_refS(1,:);
    du2dx2= ( 1i*kzP)*u_incP(2,:) + ( 1i*kzS)*u_incS(2,:) + (-1i*kzP)*u_refP(2,:) + (-1i*kzS)*u_refS(2,:);
else
