function det0=mode_Rayleigh_2R(para,DWN)
...
% det0(:,ic,1,1)=Tm1(1,1,:).*Tm1(2,2,:)-Tm1(1,2,:).*Tm1(2,1,:);
% det0(:,ic,1,2)=Tm1(1,1,:).*Tm1(2,3,:)-Tm1(1,3,:).*Tm1(2,1,:);
% det0(:,ic,1,3)=Tm1(1,1,:).*Tm1(2,4,:)-Tm1(1,4,:).*Tm1(2,1,:);
% det0(:,ic,1,4)=Tm1(1,2,:).*Tm1(2,3,:)-Tm1(1,3,:).*Tm1(2,2,:);
% det0(:,ic,1,5)=Tm1(1,2,:).*Tm1(2,4,:)-Tm1(1,4,:).*Tm1(2,2,:);
% det0(:,ic,1,6)=Tm1(1,3,:).*Tm1(2,4,:)-Tm1(1,4,:).*Tm1(2,3,:);

%reescritura para imponer Tm1 puro real (ahoro memoria)
Tm1(1,1,:)  = 2*C66*k2.*k1P(:,ic).*k1S(:,ic);%1i
Tm1(1,2,:)  = 1i*C66*ln.*k1S(:,ic);
Tm1(1,3,:)  =-k1S(:,ic).*k1P(:,ic);
Tm1(1,4,:)  = 1i*k2.*k1S(:,ic);%/1i

Tm1(2,1,:)  =-1i*C66*ln.*k1P(:,ic);
Tm1(2,2,:)  = Tm1(1,1,:);
Tm1(2,3,:)  = 1i*k2.*k1P(:,ic);%/1i
Tm1(2,4,:)  =-Tm1(1,3,:);
%coreciones en el det para tomar en cuenta Tm1 real
det0(:,ic,1,1)=   -Tm1(1,1,:).^2         -Tm1(1,2,:).*Tm1(2,1,:);
det0(:,ic,1,2)=    Tm1(1,1,:).*Tm1(2,3,:)-Tm1(1,3,:).*Tm1(2,1,:);
det0(:,ic,1,3)=1i*(Tm1(1,1,:).*Tm1(2,4,:)+Tm1(1,4,:).*Tm1(2,1,:));%1i  
det0(:,ic,1,4)= det0(:,ic,1,3);                               %1i
det0(:,ic,1,5)=    Tm1(1,2,:).*Tm1(2,4,:)-Tm1(1,4,:).*Tm1(2,2,:);%1i
det0(:,ic,1,6)=    Tm1(1,3,:).*Tm1(2,4,:)+Tm1(1,4,:).*Tm1(2,3,:);
